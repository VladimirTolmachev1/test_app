...
function sendPresent() {
    ...
    sendAjax('savePresent', {present_name: present}, message);
}
...
function getBonus() {
    ...
    sendAjax('saveBonus', {bonus_count: bonus_count}, message);
...
function getMoney() {
    if(money_count > 3000){
        ...
        sendAjax('saveMoney', {money_count: money_count}, message);
        document.getElementById('money-count').innerText = money_count;
    }
}

function sendMoney() {
    alert('send money into card action!');
}
...
function sendAjax(action, params, message){
    var url = '/?class=game&action=' + action;
